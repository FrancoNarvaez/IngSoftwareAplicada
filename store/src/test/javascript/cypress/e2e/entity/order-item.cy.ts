...
describe('OrderItem e2e test', () => {
  ...
  // const orderItemSample = {"quantity":30393,"totalPrice":9177.08,"status":"BACK_ORDER"};
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      method: 'POST',
      url: '/api/products',
      body: {"name":"vacation huzzah ugh","description":"mountain starch ridge","price":31400.89,"productSize":"L","image":"Li4vZmFrZS1kYXRhL2Jsb2IvaGlwc3Rlci5wbmc=","imageContentType":"unknown"},
    }).then(({ body }) => {
      product = body;
    });
    // create an instance at the required relationship entity:
    cy.authenticatedRequest({
      ...
      body: {"placedDate":"2025-11-30T01:00:41.448Z","status":"PENDING","code":"because gee","invoiceId":9936},
    }).then(({ body }) => {
      productOrder = body;
    });
  });
  ...
  describe('new OrderItem page', () => {
    ...
    it.skip('should create an instance of OrderItem', () => {
      cy.get(`[data-cy="quantity"]`).type('23978');
      cy.get(`[data-cy="quantity"]`).should('have.value', '23978');

      cy.get(`[data-cy="totalPrice"]`).type('22062.42');
      cy.get(`[data-cy="totalPrice"]`).should('have.value', '22062.42');
