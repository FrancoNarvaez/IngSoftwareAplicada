...
describe('Customer e2e test', () => {
  ...
  // const customerSample = {"firstName":"Luis","lastName":"Heidenreich","gender":"FEMALE","email":"_>@=b.|","phone":"729-257-0966 x68816","addressLine1":"even fast lazily","city":"Ontario","country":"Central African Republic"};
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      method: 'POST',
      url: '/api/users',
      body: {"login":"FBP@@","firstName":"Linnie","lastName":"Bartell","email":"Millie97@hotmail.com","imageUrl":"until brr physical","langKey":"aha upon i"},
    }).then(({ body }) => {
      user = body;
    });
  });
  ...
  describe('new Customer page', () => {
    ...
    it.skip('should create an instance of Customer', () => {
      cy.get(`[data-cy="firstName"]`).type('Mohammad');
      cy.get(`[data-cy="firstName"]`).should('have.value', 'Mohammad');

      cy.get(`[data-cy="lastName"]`).type('Doyle');
      cy.get(`[data-cy="lastName"]`).should('have.value', 'Doyle');

      cy.get(`[data-cy="gender"]`).select('OTHER');

      cy.get(`[data-cy="email"]`).type('!xNG,X@Qkj^.BJR~xV');
      cy.get(`[data-cy="email"]`).should('have.value', '!xNG,X@Qkj^.BJR~xV');

      cy.get(`[data-cy="phone"]`).type('1-581-753-7364 x249');
      cy.get(`[data-cy="phone"]`).should('have.value', '1-581-753-7364 x249');

      cy.get(`[data-cy="addressLine1"]`).type('boo hm');
      cy.get(`[data-cy="addressLine1"]`).should('have.value', 'boo hm');

      cy.get(`[data-cy="addressLine2"]`).type('finally farm overfeed');
      cy.get(`[data-cy="addressLine2"]`).should('have.value', 'finally farm overfeed');

      cy.get(`[data-cy="city"]`).type('North Creolacester');
      cy.get(`[data-cy="city"]`).should('have.value', 'North Creolacester');

      cy.get(`[data-cy="country"]`).type('Malawi');
      cy.get(`[data-cy="country"]`).should('have.value', 'Malawi');
